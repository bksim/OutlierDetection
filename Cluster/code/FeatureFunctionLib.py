...
class freq1_harmonics_amplitude_0(Base):
    ...
    def fit(self, data):
        ...
        for i in range(3):
            ...
            for j in range(4):
                popt, pcov = optimize.curve_fit(yfunc((j+1)*fundamental_freq), self.mjd, data)
            ...
            for j in range(4):
                data2 = np.array(data) - model(self.mjd, popts[j][0], popts[j][1], popts[j][2], (j+1)*fundamental_freq)
